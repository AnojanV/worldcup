...
echo "$($PSQL "SELECT Twin.name AS name FROM teams Twin FULL JOIN games g ON Twin.team_id=g.winner_id FULL JOIN teams Topp ON g.opponent_id=Topp.team_id WHERE year=2014 AND round='Eighth-Final' UNION ALL SELECT Topp.name FROM teams Twin FULL JOIN games g ON Twin.team_id=g.winner_id FULL JOIN teams Topp ON g.opponent_id=Topp.team_id WHERE year=2014 AND round='Eighth-Final' ORDER BY name")"
